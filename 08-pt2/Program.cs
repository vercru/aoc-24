...
foreach (var (key, positions) in grid.Where(x => x.Key != '.' && x.Value.Count > 1))
{
    foreach (var position in positions)
    {
        ...
        foreach (var otherPosition in otherPositions)
        {
            ...
            while (true)
            {
                var antinode = (x: position.x + (otherPosition.x - position.x) * idx, y: position.y + (otherPosition.y - position.y) * idx++);

                if (!WithinBounds(antinode, (0, 0), (width, length)))
                {
                    break;
                }

                if (!antinodePositions.Contains(antinode))
                {
