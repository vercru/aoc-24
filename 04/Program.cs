...
while (cursorX < width && cursorY < length)
{
    ...
    do
    {
        if (grid.TryGet(cursorX + offsets[offsetsIdx].Item1, cursorY + offsets[offsetsIdx].Item2) == 'X' &&
            ...
            grid.TryGet(cursorX + offsets[offsetsIdx+3].Item1, cursorY + offsets[offsetsIdx+3].Item2) == 'S') ++hitCount;
        offsetsIdx += 4;
    }
